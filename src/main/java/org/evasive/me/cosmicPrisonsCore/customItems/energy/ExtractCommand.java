...
public class ExtractCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String @NotNull [] strings) {
        ...
        if(strings.length == 0){
            ...
            if(!pickaxeFunctions.hasKey(mainItem, energyKey)){
                player.sendMessage("Item must have energy to extract");
                return true;
            }


            if(pickaxeFunctions.getEnergy(mainItem.getItemMeta()) == 0){
            ...
            if(pickaxeFunctions.getEnergy(meta) < amount){
                amount = pickaxeFunctions.getEnergy(meta);
            }

            pickaxeFunctions.removeEnergy(meta, amount);
            ...
            if(pickaxeFunctions.getID(meta).equals("COSMIC_ENERGY") && pickaxeFunctions.getEnergy(meta) == 0){
                player.getInventory().setItemInMainHand(newItem);
                return true;
            }

            ItemBuilder builder = ItemList.valueOf(pickaxeFunctions.getID(meta)).getItemBuilder();
